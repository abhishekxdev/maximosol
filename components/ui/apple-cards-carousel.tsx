    ...
    <CarouselContext.Provider value={{ onCardClose: handleCardClose, currentIndex }}>
      <div className="relative w-full">
        ...
        >
          ...
          <div className={cn("flex flex-row justify-start gap-3 sm:gap-4 pl-4", "max-w-7xl mx-auto")}>
  ...
  return (
    <>
        ...
        {open && (
          <div className="fixed inset-0 h-screen z-50 overflow-auto">
            ...
            <motion.div
              ...
              className="max-w-3xl mx-auto bg-white dark:bg-neutral-900 h-fit z-[60] my-6 sm:my-10 p-4 sm:p-6 md:p-10 rounded-3xl font-sans relative"
            >
              <button
              ...
              <motion.p
                ...
                className="text-lg sm:text-xl md:text-2xl font-semibold text-neutral-700 dark:text-white mt-2"
              >
                {card.title}
              </motion.p>
              <div className="py-6 sm:py-10">{card.content}</div>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="rounded-3xl bg-gray-100 dark:bg-neutral-900 h-64 sm:h-72 md:h-80 w-48 sm:w-56 md:w-64 overflow-hidden flex flex-col items-start justify-start relative z-10"
      >
        ...
        <div className="relative z-40 p-4 sm:p-6 md:p-8">
          ...
          <motion.p
            layoutId={layout ? `title-${card.title}` : undefined}
            className="text-white text-base sm:text-lg md:text-xl font-semibold max-w-xs text-left [text-wrap:balance] font-sans mt-1 sm:mt-2"
          >
            {card.title}
          </motion.p>
        </div>
          ...
          <div>
            ...
            <ul className="space-y-2 sm:space-y-3">
              {item.features.map((feature, idx) => (
                <li key={idx} className="flex items-start text-sm sm:text-base text-neutral-600 dark:text-neutral-400">
